        ...
        .forEach((entry) => {
            ...
            if (!groupedRows[key]) {
                ...
                groupedRows[key] = {
                    ...
                    lotNumber: entry.official.lotNumber,

                    gender: athlete.personalInfo.gender,

                    weightCategory: normalizedWeight,

                    displayWeightCategory: rawWeight.trim(),
...
rows.forEach((row) => {
    ...
    grouped[row.weightCategory].push(row);

});

return Object.entries(grouped).map(
    ([weightCategory, athletes]) => ({
        class: athletes[0].displayWeightCategory,
